...
```bash
# 查看所有进程资源占用情况, %CPU, %MEM 表示 CPU 和内存的占用率
top

# 查看某一个进程资源占用情况, %CPU, %MEM 表示 CPU 和内存的占用率
top -p 2913

# 查看某一个进程信息, 第 3 个和第 4 个表示 CPU 和内存占用率, 第 6 个表示物理内存使用量 (单位是 k)
ps -aux | grep 2913

# 查看内存占用前 10 名的程序信息, 第 3 个和第 4 个表示 CPU 和内存占用率, 第 6 个表示物理内存使用量 (单位是 k)
...
类似 nohup, 可让命令在后台执行. 其效果与在屏幕执行一样.

```bash
# 启动执行命令, 会进入 screen 环境
screen ./a.sh
# 安全退出, 依次执行以下两个命令, 此后 screen 命令会在后台持续运行.
ctrl + a
ctrl + d
# 查看已有的 screen 执行命令
screen -ls
# 连接进入 screen 环境
screen -r 10948.pts-1.aofeizabbix
```

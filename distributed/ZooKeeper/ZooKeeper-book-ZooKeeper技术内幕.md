...
整个集群完成 Leader 选举后, Learner 会向 Leader 进行注册, 然后进入数据同步过程.
...
Learner 向 Leader 注册的最后阶段会发送给 Leader 一个 ACKEPOCH 数据包, Leader 从该数据包中解析出 Learner 的 `currentEpoch` 和 `lastZxid`.

### 9.5.2 数据同步初始化

Leader 服务器在同步前会进行初始化, 包括以下:
- 从 ZKDatabase (内存数据库) 中提取出事务请求的 `LinkedList<Proposal> committedLog` (提议缓存队列).
- 完成 `peerLastZxid` (该 Learner 最后处理的 ZXID) 的初始化.
- 完成 `minCommittedLog` (Leader committedLog 中的最小 ZXID) 的初始化.
- 完成 `maxCommittedLog` (Leader committedLog 中的最大 ZXID) 的初始化.
...
在初始化阶段, 会优先初始化以全量同步方式来同步数据, 但是最终的同步方式需要根据 Leader 和 Learner 的情况而定.

代码实现主要在 LearnerHandler (`org.apache.zookeeper.server.quorum.LearnerHandler`) 和 Learner (`org.apache.zookeeper.server.quorum.Learner`) 中.
...
- ZooKeeper 以树作为其内存数据模型, 树上的每一个节点是最小的数据单元, 即 ZNode. ZNode 具有不同的节点特性, 同时每个节点具有一个递增的版本号, 以此可以实现分布式数据的原子性更新.

- ZooKeeper 的序列化层使用从 Hadoop 遗留下来的 Jute 组件, 该组件并不是性能最好的序列号框架, 但是目前在 ZooKeeper 中够用.

- ZooKeeper 的客户端和服务端之间会建立起 TCP 长连接来进行通信, 基于该 TCP 连接衍生出来的会话, 是客户端和服务端之间所有请求和响应交互的基石. 在会话的生命周期中, 会话管理器需要处理诸如连接断开, 重连, 会话失效等一系列问题, Leader 服务器会负责管理每个会话的生命周期, 包括会话的创建, 心跳检测和销毁等.

- 在服务器启动阶段, 会进行磁盘数据的恢复, 完成数据恢复后就会进行 Leader 选举. 一旦选举产生 Leader 后, 就开始进行集群间的数据同步, 在整个过程中, ZooKeeper 都处于不可用状态, 直到数据同步完毕才能对外提供正常服务. 在运行期间, 如果 Leader 服务器挂了或和集群中大部分服务器断开连接, 就会出发新一轮的 Leader 选举, 在新的 Leader 选举产生之前, ZooKeeper 无法对外提供服务.

- 一个正常运行的 ZooKeeper 集群, 其机器角色包括 Leader, Follower, Observer. ZooKeeper 对于客户端请求的处理, 严格按照 ZAB 协议规范来进行. 每一个服务器在启动初始化阶段都会组装一个请求处理链, Leader 能够处理所有类型的客户端请求, Follower 和 Observer 可以处理非事务请求, 而事务请求需要转发给 Leader 服务器处理, 但是, 在事务处理的过程中, Leader 和 Follower 都会进行事务日志的记录. 另外, 对于每一个事务请求, Leader 都会为其分配一个全局唯一递增的 ZXID, 以此保证事务处理的顺序性.

- ZooKeeper 通过 JDK 的 File 接口简单实现了自己的数据存储系统, 其底层数据存储包括事务日志和快照数据两部分.

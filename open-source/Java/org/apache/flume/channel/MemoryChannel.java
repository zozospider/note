...
public class MemoryChannel extends BasicChannelSemantics implements TransactionCapacitySupported {
  ...
  private class MemoryTransaction extends BasicTransactionSemantics {
    ...
    protected void doCommit() throws InterruptedException {
      ...
      if (remainingChange < 0) {
        // 在 keepAlive 时间内尝试从 bytesRemaining (控制 queue 中 events 剩余可用字节数的信号量) 中获取 putByteCounter 个许可
        // 如果在 keepAlive 时间内获取成功, 返回 true, 否则返回 false (不会一直阻塞)
        // 如果获取成功, bytesRemaining (控制 queue 中 events 剩余可用字节数的信号量) 的容量会减少 putByteCounter 个 (即 queue 中 events 剩余可用字节数减少).
        // 如果获取失败, 抛出异常
        ...
        // 如果获取成功, 继续下面逻辑
        // 在 keepAlive 时间内尝试从 queueRemaining (控制 queue 中 events 剩余可用个数的信号量) 中获取 -remainingChange 个许可
        // 如果在 keepAlive 时间内获取成功, 返回 true, 否则返回 false (不会一直阻塞)
        // 如果获取成功, queueRemaining (控制 queue 中 events 剩余可用个数的信号量) 的容量会减少 |-remainingChange| 个 (即 queue 中 events 剩余可用个数减少).
  ...
  // ps: 控制 queue 中 events 剩余可用个数的信号量 (Channel 调用 put 并 commit 会导致容量减少, 调用 take 并 commit 会导致容量增加)
  ...
  // ps: 控制 queue 中 events 存储个数的信号量 (Channel 调用 put 并 commit 会导致容量增加, 调用 take 会导致容量减少)
  // ps: 详情见 Semaphore 类的使用
  ...
  // ps: 控制 queue 中 events 剩余可用字节数的信号量 (Channel 调用 put 并 commit 会导致容量减少, 调用 take 并 commit 会导致容量增加)
  // ps: 详情见 Semaphore 类的使用
  ...
  public void configure(Context context) {
    ...
    // 根据 queue 是否为空 (即是否重新启动了 Flume 或重新加载配置文件) 和新旧 capacity 对比, 来调整 queue 和 queueRemaining (控制 queue 中 events 剩余可用个数的信号量)
    // 如果 queue 不为空
    if (queue != null) {
      try {
        // 重新调整 queue 和 queueRemaining (控制 queue 中 events 剩余可用个数的信号量)
      ...
      synchronized (queueLock) {
        ...
        // 新建一个容量为 capacity 的 Semaphore, 即 queueRemaining (控制 queue 中 events 剩余可用个数的信号量)
    ...
    // 根据 bytesRemaining (控制 queue 中 events 剩余可用字节数的信号量) 是否为空 (即是否重新启动了 Flume 或重新加载配置文件) 和新旧 byteCapacity 对比, 来调整 byteCapacity 和 bytesRemaining (控制 queue 中 events 剩余可用字节数的信号量)
    // ps: 具体逻辑与上文对 queueRemaining 的处理类似
    if (bytesRemaining == null) {
      // 重新建一个容量为 byteCapacity 的 bytesRemaining (控制 queue 中 events 剩余可用字节数的信号量)
      ...
      // 需要释放 bytesRemaining (控制 queue 中 events 剩余可用字节数的信号量) 的许可
      if (byteCapacity > lastByteCapacity) {
        // 从 bytesRemaining (控制 queue 中 events 剩余可用字节数的信号量) 中释放 (byteCapacity - lastByteCapacity) 个许可
      ...
      // 如果新 byteCapacity < 旧 byteCapacity
        ...
        try {
          // 在 keepAlive 时间内尝试从 bytesRemaining (控制 queue 中 events 剩余可用字节数的信号量) 中获取 (lastByteCapacity - byteCapacity) 个许可
          // 如果在 keepAlive 时间内获取成功, 返回 true, 否则返回 false (不会一直阻塞)
          // 如果获取失败, 不做任何处理
          ...
          // bytesRemaining (控制 queue 中 events 剩余可用字节数的信号量) 获取成功后, 记录最近的 lastByteCapacity.
          } else {
            // 记录最近的 lastByteCapacity
            lastByteCapacity = byteCapacity;
          }
  ...
  /**
   * 根据传入的新 capacity, 与旧 capacity 进行对比, 重新调整 queue 的容量, 并将 queueRemaining (控制 queue 中 events 剩余可用个数的信号量) 调整到新 queue 对应的容量.
   */
  private void resizeQueue(int capacity) throws InterruptedException {
    ...
    // 需要尝试获取 queueRemaining (控制 queue 中 events 剩余可用个数的信号量) 的许可, 并将重建立一个新的 queue.
    } else if (oldCapacity > capacity) {
      // 在 keepAlive 时间内尝试从 queueRemaining (控制 queue 中 events 剩余可用个数的信号量) 中获取 (oldCapacity - capacity) 个许可
      // 如果在 keepAlive 时间内获取成功, 返回 true, 否则返回 false (不会一直阻塞)
      // 如果获取失败, 不做任何处理
      ...
      // 如果获取成功, 则表示成功维护了新 capacity 对应的 queueRemaining (控制 queue 中 events 剩余可用个数的信号量) 的正确性.
      // queueRemaining (控制 queue 中 events 剩余可用个数的信号量) 获取成功后, 重新建立一个容量为 capacity 的 queue, 添加旧 queue 的数据并映射到原 queue 地址.
      } else {
        // 调整大小期间, 通过 queueLock 锁定保护 queue
        synchronized (queueLock) {
    ...
    // 如果旧 capacity < 新 capacity
    // 需要重建立一个新的 queue, 并释放 queueRemaining (控制 queue 中 events 剩余可用个数的信号量) 的许可.
    } else {
      // 调整大小期间, 通过 queueLock 锁定保护 queue
      synchronized (queueLock) {
        // 重新建立一个容量为新 capacity 的 queue, 将旧 queue 中的数据添加到新 queue, 再映射到原地址.
      ...
      // 从 queueRemaining (控制 queue 中 events 剩余可用个数的信号量) 中释放 (capacity - oldCapacity) 个许可
